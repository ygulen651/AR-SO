...
const listenInputFromContext = (config) => ({
  ...
  actions: {
    ...
    "post message": xstate.raise(({ event }) => (xstate.assertEvent(event, "post"), {
      type: "request",
      data: {
    ...
    "send pending messages": xstate.enqueueActions(({ enqueue }) => {
...
const createNodeMachine = () => xstate.setup({
  types: {},
  actors: {
    requestMachine: createRequestMachine(),
    listen: createListenLogic()
  ...
  actions: {
    "buffer handshake": xstate.assign({
    ...
    "process pending handshakes": xstate.enqueueActions(({ context, enqueue }) => {
